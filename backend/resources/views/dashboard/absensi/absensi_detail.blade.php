            ...
            <div class="container" data-aos="fade-left" data-aos-duration="1500">
                <div class="row">
                    <div class="col-md-3 text-center ">
                        ...
                        <span class="span bg-white py-2 border border-1 p-3 mt-2">Scan QR Code</span>
                        ...
                        @else
                            <form id="form" action="{{ route('absen.siswa.pengajar') }}" method="POST">
                                @csrf
                                <input type="text" name="kode_siswa" id="kode_siswa" hidden>
                                <input type="text" name="absensi_id" id="absensi_id" value="{{ $absensi_id }}" hidden>
                                        ...
                                        @foreach ($item as $item)
                                            <tr>
                                                ...
                                                <td class="text-xs text-secondary opacity-7 align-middle">
                                                    ...
                                                    @else
                                                        <form
                                                            action="{{ route('absensi_detail.setHadir.pengajar', $item->id) }}"
                                                            method="POST" style="display: inline-block;">
                                                            @method('POST')
                                                            @csrf
                                                            <button type="submit" class="btn btn-sm btn-success"
                                                        ...
                                                        <form
                                                            action="{{ route('absensi_detail.setIzin.pengajar', $item->id) }}"
                                                            method="POST" style="display: inline-block;">
                                                            @method('POST')
                                                            @csrf
                                                            <button type="submit" class="btn btn-sm btn-secondary"
                                                        ...
                                                        <form
                                                            action="{{ route('absensi_detail.setSakit.pengajar', $item->id) }}"
                                                            method="POST" style="display: inline-block;">
                                                            @method('POST')
                                                            @csrf
                                                            <button type="submit"
                                                        ...
                                                        <form
                                                            action="{{ route('absensi_detail.setAlpa.pengajar', $item->id) }}"
                                                            method="POST" style="display: inline-block;">
                                                            @method('POST')
                                                            @csrf
                                                            <button type="submit"
